...
open class BalloonMarker: MarkerImage
{
    ...
    fileprivate var _dateSize: CGSize = CGSize()
    ...
    {
        guard let data = entry.data as? Transaction , let amount = data.amount, let timestamp = data.timestamp else { return }
        let y = amount > 0 ? "+\(String(format: "%.1f", entry.y))" : "-\(String(format: "%.1f", entry.y))"
        if amount < 0 {
            self.color = .green
        }else {
            self.color = .navy
        }
        setLabel("\(y)\n\(timestamp.formatted("MMM d, hh:mm"))")
    }
